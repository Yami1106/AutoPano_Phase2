...
def extract_zip(data_root: Path):
    for split in ["Train", "Val"]:
        ...
        if not zip_path.exists():
            raise FileNotFoundError(f"Expected either {split_dir} or {zip_path} to exist")
...
def process_split(split_name: str, cfg: DataGenConfig):
    ...
    for idx, p in enumerate(img_paths):
        ...
        save_outputs_for_image(out, img_stem=img_stem, patch_dir=patch_dir, overlay_dir=overlay_dir)
